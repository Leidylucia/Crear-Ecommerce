    ...
    <div class="bg-white rounded-lg shadow-lg mb-6">
        <div class="px-6 py-2 flex justify-between items-center">
            ...
            <div class="grid grid-cols-2 border border-gray-200 divide-x divide-gray-200 text-gray-500">
                ...
                <i class="fas fa-list p-3 cursor-pointer {{ $view == 'list' ? 'text-green-500' : '' }}"
                ...
                @foreach ($category->subcategories as $subcategory)
                    <li class="py-2 text-sm">
                        <a wire:click="$set('subcategoria','{{ $subcategory->name }}')"
                            class="cursor-pointer hover:text-green-500 capitalize {{ $subcategoria == $subcategory->name ? 'text-green-500 font-semibold' : '' }}">
                            {{ $subcategory->name }}
                        </a>
                    </li>
                @endforeach
            </ul>
            <h2 class="font-semibold text-center mt-2 mb-2">
                Marcas
            </h2>
            <ul class="divide-y divide-gray-200 ">
                @foreach ($category->brands as $brand)
                    <li class="py-2 text-sm">
                        <a wire:click="$set('marca','{{ $brand->name }}')"
                            class="cursor-pointer hover:text-green-500 capitalize {{ $marca == $brand->name ? 'text-green-500 font-semibold' : '' }}">
                            {{ $brand->name }}
                        </a>
                    </li>
                @endforeach
            ...
            @if ($view == 'grid')
                <ul class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach ($products as $product)
                        <li class="bg-white rounded-lg shadow mt-3">
                            <article>
                                <figure>
                            ...
                            <article class="flex">
                                ...
                                <div class="flex-1 py-4 px-6 flex flex-col">
                                   ...
                                   <div class="mt-auto mb-6">
                                       <x-danger href="{{route('products.show',$product)}}">
                                           Más información
                                       </x-danger>
                                   </div>
